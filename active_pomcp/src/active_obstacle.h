 ...
 */
template<typename T>
class ACTIVE_OBSTACLE
{
public:
    using trace_t = T;
    using PARAMS = typename TRACE_EXPERIMENT<trace_t>::PARAMS;

    ACTIVE_OBSTACLE(const SIMULATOR &real, const SIMULATOR &simulator,
                    PARAMS &expParams,
                    MCTS::PARAMS &searchParams, const BOUNDS &sol);
...
template<typename T>
ACTIVE_OBSTACLE<T>::ACTIVE_OBSTACLE(const SIMULATOR &real,
                                 const SIMULATOR &simulator,
                                 PARAMS &expParams,
                                 MCTS::PARAMS &searchParams,
                                 const BOUNDS &s)
    : Real(real), Simulator(simulator),
      ExpParams(expParams), SearchParams(searchParams), sol(s) {
...
template<typename T>
void ACTIVE_OBSTACLE<T>::Run(trace_t &trace)
{
    Timer timer;
    ...
    for (t = 0; t < ExpParams.NumSteps; t++)
    {
        ...
        terminal = Real.Step(mcts.GetRoot(), *state, action, observation, reward, sol);
...
template<typename T>
void ACTIVE_OBSTACLE<T>::MultiRun(trace_t &trace)
{
...
template<typename T>
void ACTIVE_OBSTACLE<T>::BuildTrace(trace_t &trace)
{
