...
public:
    ...
    void Set(double count, double value) // Set node count and value
    {
        Count = count;
        Total = value * count;
    }

    void Add(double totalReward)    // Adds a visit to the node (count+1, total+totalReward)
    {
        Count += 1.0;
        Total += totalReward;
    }

    void Add(double totalReward, COUNT weight)  // Add A weighted visit to the node
    {
        Count += weight;
        Total += totalReward * weight;
    }

    double GetValue() const     // Get the node value (total/count)
    {
        return Count == 0 ? Total : Total / Count;
    }

    COUNT GetCount() const      // Get node count (number of visits)
    {
        return Count;
    }
...
public:
    ...
    void DisplayValue(HISTORY& history, int maxDepth, std::ostream& ostr) const;    // Prints QNODE value
